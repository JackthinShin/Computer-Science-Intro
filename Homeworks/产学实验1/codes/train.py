...
def model_fn(FLAGS, objective, optimizer, metrics):
    ...
    base_model.summary()
    base_model.load_weights(FLAGS.weights, by_name=True, skip_mismatch=True)
    for layer in base_model.layers:
        layer.trainable = False
    x = base_model.output
    x = Flatten()(x)
    predictions = Dense(FLAGS.num_classes, activation='softmax', name="output")(x)
...
class LossHistory(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        global curr_accuracy
        acc = logs.get('val_accuracy')
        if(acc > curr_accuracy):
            from save_model import save_pb_model
            save_pb_model(self.FLAGS, self.model, acc)
            # self.model.save_weights(os.path.join(self.FLAGS.train_url, 'weights_%.4f.h5' % (acc)))
            curr_accuracy = acc
